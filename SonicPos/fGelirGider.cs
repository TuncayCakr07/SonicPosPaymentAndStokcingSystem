...
namespace SonicPos
{
    public partial class fGelirGider : Form
    {
        ...
        {
            lGelirGider.Text=gelirgider + " Ekleme İşlemi Yapılıyor";
        ...
        private void bEkle_Click(object sender, EventArgs e)
        {
            if (cmbOdemeTuru.Text!="")
            {
                if (tNakit.Text!="" && tKart.Text!="")
                {
                    using (var db=new SonicPosDbEntities())
                    {
                        ...
                        io.KrediKartı=Islemler.DoubleYap(tKart.Text);
                        if (gelirgider=="GELİR")
                        {
                            io.Gelir = true;
                            io.Gider = false;
                        ...
                        db.SaveChanges();
                        MessageBox.Show(gelirgider + " İşlemi Kaydedildi.","Bilgi",MessageBoxButtons.OK,MessageBoxIcon.Information);
                        tNakit.Text = "0";
                        tKart.Text = "0";
                        tAciklama.Clear();
                        cmbOdemeTuru.Text = "";
            ...
            {
                MessageBox.Show("Lütfen Ödeme Türünü Seçiniz!", "Uyarı!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
